...
| `VAULT_TRADE_ALLOWLIST_MODE` | `block` | How to treat an unknown payee that is NOT allowlisted and NOT tainted (a legitimate-looking first-time recipient). `block` = strict. `warn` = flag it but let it through, so the allowlist doesn't over-block real new payees. A **tainted** recipient (sourced from an untrusted tool response — the actual injection signal) always hard-blocks regardless of this setting. |
| `VAULT_TRADE_TRUSTED_TOOLS` | empty | Regexes matching tool names whose responses are a **trusted** provenance source (e.g. the user's own account/history tool). A recipient whose taint lineage is entirely trusted tools is legitimate provenance and does NOT hard-block — this stops the guard over-blocking addresses the agent read from a source you trust. Any untrusted lineage still hard-blocks. Empty = every source untrusted (conservative default, current behavior). |
